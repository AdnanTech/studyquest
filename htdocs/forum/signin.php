...
else
{
	...
	else
	{
		...
		*/
		...
		{
			// form has been posted without errors, so save it
			// mysql_real_escape_string prevents SQL injection
			// sha1 function which hashes the password
			$sql = "SELECT user_id, user_name, user_level FROM users WHERE user_name = '" . $_POST['user_name'] . "' AND
			...
			{
				// query was successfully executed, there are 2 possibilities
				// 1. the query returned data, the user can be signed in
				// 2. the query returned an empty result set, the credentials were wrong
				if(mysqli_num_rows($result) == 0)
				{
					echo '<br><font style="font-size: 14px;">You have supplied a wrong user/password combination. Please <a href="signin.php">try again.</a></font><br><br>';
				}
				else
				{
