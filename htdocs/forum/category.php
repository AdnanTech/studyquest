...
if(!$result)
{
	echo '<br><font style="font-size: 14px;">The category could not be displayed, please try again later.</font><br><font style="font-size: 14px;">' . mysqli_error($connect_database) . '</font><br>';
}
else
{
	...
	else
	{
		...
		// get topic data
		$get_id = $_GET['id'];
		$sql = "SELECT topic_id, topic_subject, topic_date, topic_cat FROM topics WHERE topic_cat = '$get_id'	ORDER BY topic_date DESC";
			...
			else
			{
				...
				{				
					echo '<tr>';
						...
						echo '<font style="font-size: 14px;"><a href="topic.php?id=' . $row['topic_id'] . '">' . htmlentities($row['topic_subject']) . '</a><br></font>';						echo '</td>';
