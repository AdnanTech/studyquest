...
else
{
	...
	{	
		...
		else
		{
			...
			{
				// no categories found, so a topic can't be posted
			...
			else
			{
				...
				<br><font style="font-size: 14px;">Subject: </font><input type="text" name="topic_subject"></input><br><br>
			...
			echo '<font style="font-size: 14px;">Message: </font><br><textarea style="resize:none;" name="post_content" rows="10" cols="70" wrap="hard"></textarea><br>
<br>
	...
	else
	{
			...
			$sql = "INSERT INTO topics(topic_subject, topic_date, topic_cat, topic_by)
				   VALUES('" . addslashes($_POST['topic_subject']) . "', NOW(), " . $_POST['topic_cat'] . ", " . $_SESSION['user_id'] . ")";
			...
			else
			{
				...
				$sql = "SELECT topic_id FROM topics ORDER BY topic_date DESC LIMIT 1";
				$result = mysqli_query($connect_database, $sql);
          ...
          $topicid = $row['topic_id'];					
				...
				$sql = "INSERT INTO posts(post_content, post_date, post_topic, post_by)
						VALUES ('" . addslashes($_POST['post_content']) . "', NOW(), " . $topicid . ", " . $_SESSION['user_id'] . " )";
				$result = mysqli_query($connect_database,$sql);
