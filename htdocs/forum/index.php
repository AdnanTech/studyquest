...
$sql = "SELECT categories.cat_id, categories.cat_name, categories.cat_description, COUNT(topics.topic_id) AS topics 
				FROM categories	LEFT JOIN topics ON topics.topic_id = categories.cat_id 
	...
	else
	{
		...
		{				
			...
			$topicsql = "SELECT topic_id, topic_subject, topic_date, topic_cat
									FROM topics WHERE topic_cat = " . $row['cat_id'] . " ORDER BY topic_date DESC LIMIT 1";
			...
			else
			{
				if(mysqli_num_rows($topicsresult) == 0)
				{
					...
					while($topicrow = mysqli_fetch_assoc($topicsresult)) {
			 		echo '<font style="font-size: 14px;"><a href="topic.php?id=' . $topicrow['topic_id'] . '">' . $topicrow['topic_subject'] . '</a><br> at ' . date('d-m-Y H:i', 
		 			strtotime($topicrow['topic_date'])) . '</font>';
				}						
				}					
			}
			echo '</td>';
